import { useEffect } from "react";
import styles from "./StackIcon.module.css";

import AOS from "aos";
import "aos/dist/aos.css";

export default function StackIcon() {
  useEffect(() => {
    AOS.init();
  }, []);
  return (
    <div className={styles.skill_Container}>
      <div
        className={styles.skill_iconContainer}
        data-aos-anchors="#Stack"
        data-aos="flip-right"
        data-aos-duration={2000}
      >
        <div className={styles.skill_iconArea_1}>
          <div
            className={styles.iconHTML_2}
            data-aos-anchors="#Stack"
            data-aos="zoom-in"
            data-aos-duration={1000}
          ></div>
          <h3
            data-aos-anchors="#Stack"
            data-aos="fade-left"
            data-aos-duration={1000}
          >
            HTML
          </h3>
        </div>
        <div className={styles.skill_iconArea_2}>
          <h3
            data-aos-anchors="#Stack"
            data-aos="fade-right"
            data-aos-duration={1000}
          >
            CSS
          </h3>
          <div
            className={styles.iconCSS_2}
            data-aos-anchors="#Stack"
            data-aos="zoom-in"
            data-aos-duration={1000}
          ></div>
        </div>
        <div className={styles.skill_iconArea_3}>
          <div
            className={styles.iconJS_2}
            data-aos-anchors="#Stack"
            data-aos="zoom-in"
            data-aos-duration={1000}
          ></div>
          <h3
            data-aos-anchors="#Stack"
            data-aos="fade-left"
            data-aos-duration={1000}
          >
            Java <br />
            Script
          </h3>
        </div>
      </div>

      <div
        className={styles.skill_iconContainer}
        data-aos-anchors="#Stack"
        data-aos="flip-right"
        data-aos-duration={2000}
      >
        <div
          className={styles.skill_iconArea_1}
          style={{ backgroundColor: "#9AE1F7" }}
        >
          <div
            className={styles.iconReactJS_2}
            data-aos-anchors="#Stack"
            data-aos="zoom-in"
            data-aos-duration={1000}
          ></div>
          <h3
            data-aos-anchors="#Stack"
            data-aos="fade-left"
            data-aos-duration={1000}
          >
            React <br />
            JS
          </h3>
        </div>
        <div
          className={styles.skill_iconArea_2}
          style={{ backgroundColor: "#78C1DE" }}
        >
          <h3
            data-aos-anchors="#Stack"
            data-aos="fade-right"
            data-aos-duration={1000}
          >
            React <br />
            Native
          </h3>
          <div
            className={styles.iconRN_2}
            data-aos-anchors="#Stack"
            data-aos="zoom-in"
            data-aos-duration={1000}
          ></div>
        </div>
      </div>

      <div
        className={styles.skill_iconContainer}
        data-aos-anchors="#Stack"
        data-aos="flip-right"
        data-aos-duration={2000}
      >
        <div
          className={styles.skill_iconArea_1}
          style={{ backgroundColor: "#7FDAAD" }}
        >
          <div
            className={styles.iconFigma_2}
            data-aos-anchors="#Stack"
            data-aos="zoom-in"
            data-aos-duration={1000}
          ></div>
          <h3
            data-aos-anchors="#Stack"
            data-aos="fade-left"
            data-aos-duration={1000}
          >
            Figma
          </h3>
        </div>
        <div
          className={styles.skill_iconArea_2}
          style={{ backgroundColor: "rgb(66, 66, 66)" }}
        >
          <h3
            style={{ color: "white" }}
            data-aos-anchors="#Stack"
            data-aos="fade-right"
            data-aos-duration={1000}
          >
            Git Hub
          </h3>
          <div
            className={styles.iconGit_2}
            data-aos-anchors="#Stack"
            data-aos="zoom-in"
            data-aos-duration={1000}
          ></div>
        </div>
      </div>
    </div>
  );
}
